...
不建议把本脚本放入全局扩展脚本，因为不同订阅的策略组名称可能不同。推荐将脚本绑定到需要校园
规则的单个订阅，订阅更新后 Clash Verge Rev 会再次应用该脚本。

### 安装步骤

1. 打开 [`scripts/clash-verge-extension.js`](scripts/clash-verge-extension.js)，复制完整内容。
2. 在 Clash Verge Rev 的配置列表中，右键需要添加校园规则的订阅，选择“编辑扩展/脚本”，
   将脚本粘贴进去。不同版本中的菜单名称可能略有差异。
3. 查看脚本顶部的 `SETTINGS`。`fallbackGroup` 默认留空，请填写该订阅中真实存在的主策略组名称：

   ```javascript
   // 填写订阅中的主策略组名称；留空时“校园服务”中只有 DIRECT。
   fallbackGroup: "",
   ```

   例如订阅中的主策略组名为 `节点选择`，则修改为：

   ```javascript
   fallbackGroup: "节点选择",
   ```

4. 保存脚本，重新启用扩展或刷新当前订阅，然后在“代理”页面找到 `🏫校园服务`：
   - 连接南航校园网时选择 `DIRECT`；
   - 离开校园网时可选择脚本配置的备用代理组。
